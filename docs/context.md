...
- `apps/extension`: extension Chrome MV3 (popup) para operacion comercial diaria.
  ...
  - Popup con `BACKEND_URL` gestionado internamente (persistido en storage local con fallback seguro a produccion); si la URL guardada deja de responder, popup/panel reintentan contra produccion y resincronizan el valor. La edicion manual queda oculta en UI final para evitar errores operativos.
